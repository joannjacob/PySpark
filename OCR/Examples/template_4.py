...
for scale in np.linspace(0.2, 1.0, 20)[::-1]:
    ...
    # cv2.TM_CCOEFF) (_, minVal, _, minLoc) = cv2.minminLoc(result)
    # if we have found a new maximum correlation value, then update
    # the found variable if found is None or minVal > found[0]:
    ...
    if visualize:
        ...
        cv2.rectangle(clone, (minLoc[0], minLoc[1]),
                      (minLoc[0] + w, minLoc[1] + h), (0, 0, 255), 2)
...
(endX, endY) = (int((minLoc[0] + w) * r), int((minLoc[1] + h) * r))
...
image = image[top_left[1]:top_left[1]+h, bottom_right[0]-w:bottom_right[0]]

cv2.imwrite("template_4.jpg", image)
